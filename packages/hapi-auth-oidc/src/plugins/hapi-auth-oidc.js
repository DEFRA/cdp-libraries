...
export const HapiAuthOidcPlugin = {
  ...
  register: async function (server, options) {
    ...
    const discoveryRequestOptions = {
      ...
      execute: [
        ...(oidc.discoveryRequestOptions?.execute ?? []),
        ...(oidc.useHttp ? [openid.allowInsecureRequests] : [])
      ]
    }
...
/**
 * Rewrites a URL so it appears as if it originated from an external base URL.
 *
 * The path, query string, and hash are preserved from the original URL,
 * while protocol, hostname, and port are taken from the external URL.
 **
 * @param {string | URL} url
 *   The original URL (or URL string) to rewrite.
 * @param {string | URL} external
 *   The external base URL providing protocol, hostname, and port.
 * @returns {URL}
 *   A new URL object using the external origin and original path.
 */
